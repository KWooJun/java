...
public class ExceptionStudy {
    ...
    //1.myParseInt 가 메소드인지는(str1)을 보고 알수 있다.
    //2.myParseInt 앞에 . 을 붙이지 않았기 떄문에 같은 class 안에 있는 메소드
    //3. 2번 내용과 일맥상통하게 .을 붙이지 않았다는 것은 객체화를 하지 않았다는 의미 고로 static 메소드
    //4.myParseInt(str1) 앞에 "int int Val1 =" 코드가 있었기 떄문에 리턴 메소드
    //5.리턴타입은 int 라는 것을 알 수 있었다.
    // 6.파라미터는 str1의 타입이 String 이기 떄문에 파라미터 타입도 String 이다.
}
