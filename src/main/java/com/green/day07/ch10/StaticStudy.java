...
public class StaticStudy {
    public static void main(String[] args) {
        ...
        /*
        객체마다 멤버필드의 공간이 각각 생기길 원하면 non-static(인스턴스 멤버 필드)로 해야한다.
        (각 객체마다 다른 값을 저장하고 싶다면)

        static 을 사용할 수 있으면 (위 두줄의 내용과 반대되는 경우) 적극 사용하는 걸 추천!
        왜냐하면 일단 실행 속도가 빠르고 사용하기 편하다.(객체 생성이 필요하지 않기 때문에)
         */
...
//        math.random();
        /*
        이렇게 두줄을 통해서 메소드를 호출할 필요가 없기 때문에 static 메소드가 사용이 편하다
        그리고 Math 클래스는 객체화를 할 수 없다.
        이유는 기본 생성자 접근 레벨을 private 으로 세팅했기 떄문
         */
...
class StaticNumBox{
    ...
    static void sum(){
        System.out.println(n1 + n2);
        //System.out.println(n3 + n4); //static 메소드에서 인스턴스 멤버 필드 집근 불가
...
class NonStaticNumBox{
    ...
    void sum(){
        ...
        hello();
        /*
        인스턴스 메소드에서 멤버 메소드 호출 가능하다.
        왜 가능 하나 > sum() 메소드가 호출이 된다는 것은 NonStaticNumBox 가 객체화 되었다는 것이고
        static 는 이미 메모리에 올라가 있기 때문에 사용이 가능하다.
         */
    }
